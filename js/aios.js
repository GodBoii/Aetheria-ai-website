...
const SUBSCRIPTION_CHANGEABLE_STATUSES = new Set(['authenticated', 'active']);
const PAID_SUBSCRIPTION_PLANS = new Set(['pro', 'elite']);
...
export class AIOS {
    ...
    async handleSignup() {
        ...
        const phoneNumber = this.elements.signupForm.querySelector('#signupPhone').value;
        ...
        if (!name.trim() || !phoneNumber.trim() || !email.trim() || !password) {
            this.elements.signupError.textContent = 'Name, email, mobile number, and password are required.';
            return;
        }
    ...
    getPlanActionState(plan, summary) {
        ...
        const canCreateSubscription = Boolean(summary?.can_create_subscription);
        const canChangePaidPlan = PAID_SUBSCRIPTION_PLANS.has(currentPlanType)
            && PAID_SUBSCRIPTION_PLANS.has(planType)
            && currentPlanType !== planType
            && SUBSCRIPTION_CHANGEABLE_STATUSES.has(status);
        ...
        if (!canCreateSubscription && !canChangePaidPlan) {
            return {
                label: 'Unavailable',
                note: SUBSCRIPTION_CHANGEABLE_STATUSES.has(status)
                    ? 'Plan changes are unavailable for this subscription.'
                    : 'Existing subscription in progress.',
                disabled: true,
            };
        }
        ...
        return {
            ...
            note: canChangePaidPlan ? 'Plan change handled on your current subscription.' : 'Upgrade instantly using Razorpay Checkout.',
            disabled: false,
        };
    }
    ...
    async startPlanUpgrade(planType) {
        ...
        const currentPlanType = String(this.subscriptionSummary?.plan_type || 'free').toLowerCase();
        const status = String(this.subscriptionSummary?.subscription_status || 'none').toLowerCase();
        const isPaidPlanChange = PAID_SUBSCRIPTION_PLANS.has(currentPlanType)
            && PAID_SUBSCRIPTION_PLANS.has(normalizedPlan)
            && currentPlanType !== normalizedPlan
            && SUBSCRIPTION_CHANGEABLE_STATUSES.has(status);
        if (!this.subscriptionSummary.can_create_subscription && !isPaidPlanChange) {
        ...
        this.checkoutTimeoutId = window.setTimeout(() => {
            if (!this.isCheckoutInProgress) return;
            this.showNotification('Checkout is taking longer than expected. You can retry when ready.', 'warning');
        }, 30000);
        ...
        try {
            const response = await fetch(`${API_BACKEND_URL}/api/subscription/create`, {
                ...
                body: JSON.stringify({
                    ...
                    idempotency_key: IdempotencyKeyGenerator.generate(`subscription-${normalizedPlan}`),
                }),
            });
            const payload = await response.json().catch(() => ({}));
            ...
            checkoutSubscriptionId = payload?.subscription_id || '';
            if (payload.checkout_required === false) {
                const changeType = String(payload.change_type || 'plan change').replace('_', ' ');
                this.showNotification(`Subscription ${changeType} saved.`, 'success');
                await this.loadUsageData();
                this.closeSubscriptionModal();
                return;
            }
            const completed = await this.launchRazorpayCheckout(payload);
            if (!completed) {
                await this.cleanupIncompleteSubscription(payload?.subscription_id);
                await this.loadUsageData();
            }
        } catch (error) {
            console.error('[Subscription] Failed to start checkout:', error);
            await this.cleanupIncompleteSubscription(checkoutSubscriptionId);
            await this.loadUsageData();
            this.showNotification(error.message || 'Failed to start checkout.', 'error');
        } finally {
            if (this.checkoutTimeoutId) {
                window.clearTimeout(this.checkoutTimeoutId);
                this.checkoutTimeoutId = null;
            }
            this.isCheckoutInProgress = false;
            if (this.subscriptionSummary) {
    ...
    async launchRazorpayCheckout(createPayload) {
        ...
        const checkoutOptions = this.buildRazorpayCheckoutOptions(createPayload);
    ...
    buildRazorpayCheckoutOptions(createPayload) {
        const profile = this.subscriptionSummary?.profile || {};
        const options = {
            key: createPayload?.key_id,
            subscription_id: createPayload?.subscription_id,
            recurring: 1,
            name: 'Aetheria AI',
            ...
            notes: {
                plan_type: createPayload?.plan_type || '',
                app: 'aetheria_ai_web',
            },
        };
    ...
    async cleanupIncompleteSubscription(subscriptionId) {
        ...
        try {
            ...
            const response = await fetch(`${API_BACKEND_URL}/api/subscription/cleanup`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ subscription_id: normalizedId }),
            });
            const payload = await response.json().catch(() => ({}));
            if (!response.ok || !payload?.ok) {
                console.warn('[Subscription] Cleanup failed:', payload?.error || response.status);
            }
        } catch (error) {
            console.warn('[Subscription] Cleanup request failed:', error);
        }
    }

    normalizeRazorpayError(error, fallbackMessage) {
        if (!error) return new Error(fallbackMessage);
        if (error instanceof Error && error.message) return error;

        const raw = error?.message || error?.code || error?.description || error;
        if (typeof raw === 'string') {
            try {
                const parsed = JSON.parse(raw);
                const description = parsed?.description || parsed?.error?.description || parsed?.reason;
                if (description) return new Error(description);
            } catch (_) {
                if (raw.trim()) return new Error(raw);
            }
        }

        const description = error?.description || error?.error?.description || error?.reason;
        return new Error(description || fallbackMessage);
    }
