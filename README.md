...
This project uses **CodeRabbit AI** for automated code review and quality assurance.

### Features
- **Full Repository Analysis**: Reviews entire codebase, not just changed files
- **Multi-Language Support**: Python, JavaScript, HTML, CSS
- **Security Scanning**: Detects vulnerabilities and hardcoded secrets
- **Performance Analysis**: Mobile optimization and PWA compliance
- **Automated PR Reviews**: Integrated with GitHub pull requests

### Usage
1. **Automatic Reviews**: All PRs trigger comprehensive analysis
2. **Manual Trigger**: Add `@coderabbitai full review` to any PR comment
3. **Configuration**: See `.coderabbitrc.yaml` for detailed settings
4. **Documentation**: Complete setup guide in `CODERABOT_SETUP.md`

### Quick Commands
```bash
# Trigger full repository review
...
@coderabbitai review files:python-backend/api.py,js/chat.js
```
...
- **[CodeRabbit Setup Guide](CODERABOT_SETUP.md)** - Complete AI code review configuration
- **[Android Build Guide](IOS_BUILD_GUIDE.md)** - Mobile deployment instructions
- **[Docker Configuration](DOCKER.md)** - Container setup and deployment
- **[Supabase Configuration](PWA_SUPABASE_CONFIGURATION.md)** - Database and auth setup
